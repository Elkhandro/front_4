...
function FilterTabs({ activeFilter, onFilterChange, technologies }) {
  const filters = [
    { key: "all", label: "Все", count: technologies.length },
    {
      key: "completed",
      label: "Выполнены",
      count: technologies.filter((t) => t.status === "completed").length,
    },
    {
      key: "in-progress",
      label: "В процессе",
      count: technologies.filter((t) => t.status === "in-progress").length,
    },
    {
      key: "not-started",
      label: "Не начаты",
      count: technologies.filter((t) => t.status === "not-started").length,
    },
  ];
